...
<input type="text" class="form-control" placeholder="Ticketid" aria-label="Ticketid"
aria-describedby="ticketid-addon" name="ticketid"
value="@isset($ticketid){{$ticketid}}@endisset{{old('ticketid')}}">
</div>
...
<td><a href="{{$ticket->getService->href_link}}{{$ticket->ticketid}}"
target="_blank">{{$ticket->ticketid}}</a></td>
<td>{{$ticket->subject}}</td>
<td>
<a class="btn btn-primary" href="{{route('ticket.show',['id'=>$ticket->id])}}"
title="Show {{$ticket->ticketid}}"><i class="far fa-folder-open"></i></a>
<form action="{{route('boss.ticket.destroy',$ticket->id)}}" method="post"
class="float-right">
@method('DELETE')
@csrf
<button type="submit" class="btn btn-dark"
onclick="return confirm('Видалення тыкета. Ви впевнені?')"
